...
int main(void)
{
	system_clock::time_point begin_tp = system_clock::now();
	int N;
	while (cin >> N)
	{
		...
		if (ok && !action.empty())
			for (auto i = action.begin(); i != action.end(); i++)
				printf("%s\n", i->c_str());
		printf("FINISH\n");
		system_clock::time_point end_tp = system_clock::now();
		nanoseconds dur = duration_cast<nanoseconds>(end_tp - begin_tp);
